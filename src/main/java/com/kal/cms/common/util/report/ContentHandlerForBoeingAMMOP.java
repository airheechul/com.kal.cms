...
public class ContentHandlerForBoeingAMMOP extends CustomContentHandler{
	...
	public ContentHandlerForBoeingAMMOP(DTDHandler dtdHandler, String outputPath, String csvPrefix, ReportInfo report, Writer writer){
	...
	public void startElement(String uri, String localName, String qName,
		...
		if(elemNameTask.equalsIgnoreCase(localName)){
			...
			if(attributes!=null){
				...
				if(tempMap.get("CONFLTR") == null || "".equals(tempMap.get("CONFLTR"))){
					sRoute_no = report.getcMODEL() + "_" + tempMap.get("CHAPNBR") + "-" + tempMap.get("SECTNBR") + "-" + tempMap.get("SUBJNBR") + "-" + tempMap.get("FUNC") + "-" + tempMap.get("SEQ");
					sGrp_Op = "KE." + report.getcMODEL() + ".GRP." + tempMap.get("CHAPNBR") + "-" + tempMap.get("SECTNBR") + "-" + tempMap.get("SUBJNBR") + "." + tempMap.get("FUNC") + "." + tempMap.get("SEQ") + ".";
				} else {
				...
				/*
		        If TASKCONFLTR = "" Then
                    sRoute_no = cMODEL & "_" & TASKCHAPNBR & "-" & TASKSECTNBR & "-" & TASKSUBJNBR & "-" & TASKFUNC & "-" & TASKSEQ
                    sGrp_Op = "KE." & cMODEL & ".GRP." & TASKCHAPNBR & "-" & TASKSECTNBR & "-" & TASKSUBJNBR & "." & TASKFUNC & "." & TASKSEQ & "."
                Else
		...
		if(innerTaskFlag && elemNameSubTask.equalsIgnoreCase(localName)){
			...
			if(attributes!=null){
				...
				if(tempMap.get("CONFLTR") == null || "".equals(tempMap.get("CONFLTR"))){
					sOperation_no = "KE." + report.getcMODEL() + ".AMM." + tempMap.get("CHAPNBR") + "-" + tempMap.get("SECTNBR") + "-" + tempMap.get("SUBJNBR") + "." + tempMap.get("FUNC") + "." + tempMap.get("SEQ") + ".";
				} else {
					sOperation_no = "KE." + report.getcMODEL() + ".AMM." + tempMap.get("CHAPNBR") + "-" + tempMap.get("SECTNBR") + "-" + tempMap.get("SUBJNBR") + "." + tempMap.get("FUNC") + "." + tempMap.get("SEQ") + "." + tempMap.get("CONFLTR") + (tempMap.get("VARNBR")==null?"":tempMap.get("VARNBR"));
				}
		...
		try {
			...
			if(innerPARA && "PARA".equalsIgnoreCase(localName)) {
                ...
                if (!"D".equals(SUBCHG) && iParaCnt == 0 ){
                    ...
                    sStep += 10;
                    sDescription = sDescription.trim().replaceAll(",", "");
	...
	//TODO 구 VB Boing_AMM_OP
	/*
	Public Sub Boing_AMM_OP(ByRef Nodes As MSXML2.IXMLDOMNodeList, ByVal Indent As Integer)
	...
	'                    "Description, Remarks, Chapter, Section, Subject"
	    ...
	    For Each xNode In Nodes
	           ...
	           Select Case UCase(xNode.nodeName)
	                  ...
	                  Case "TITLE":
	                        ...
	                        If xNode.parentNode.nodeName = "TASK" Then
	                           rRouteTitle = """" & Replace(Trim(xNode.Text), """", "'") & """"
	                        End If
	                  ...
	                  Case "SUBTASK":
	                        ...
	                        For k = 0 To xNode.Attributes.length - 1
	                            Select Case UCase(xNode.Attributes(k).nodeName)
	                                   ...
	                                   Case "VARNBR"
	                                        SUBVARNBR = xNode.Attributes(k).Text
	                                   Case "SECTNBR"
	        ...
	        If xNode.hasChildNodes Then   '하위노드 존재시 재호출
	           ...
	           Else
	              Boing_AMM_OP xNode.childNodes, Indent
	           End If
	        End If
